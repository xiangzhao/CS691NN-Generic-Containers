...
void test4() {
	srand(clock());
	std::list<random_operation_t> ops(TEST4);
	...
	ait++;	
	clock_t t1 = clock();
	for(std::list<random_operation_t>::iterator iop=ops.begin();iop!=ops.end();iop++) {
		...
		switch(op.operation) {
			case 0:
				...
				break;
			case 1:
				ait = adsequence.insert(ait,op.what);
				break;
			case 2:
				adsequence.sort();
				break;
	...
	clock_t t2 = clock();
	std::cout << "Time for AdaptiveSequence: " << (float) (t2 - t1) << std::endl;
	//std::deque test
	...
	for(std::list<random_operation_t>::iterator iop=ops.begin();iop!=ops.end();iop++) {
		...
		switch(op.operation) {
			case 0:
				...
				break;
			case 1:
				lit = l.insert(lit,op.what);
				break;
			case 2:
				l.sort();
				break;
			case 3:
				typename std::list<int>::iterator iter = l.begin();
