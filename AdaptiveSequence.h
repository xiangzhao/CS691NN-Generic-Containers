...
template<T,class Allocator=allocator<T> > class AdaptiveSequence {
protected:
	enum representation_t { LIST, VECTOR, DEQUEUE };
	typedef union {
		std::list<T>* list;
		std::vector<T>* vector;
		...
		ContentsADT(representation_t repr) {
			switch(repr) {
				...
				case VECTOR:
					...
					break;
				case DEQUEUE:
					contents.dequeue = new std::dequeue<T>();
					break;
			}
		}
		~ContentsADT() {
...
public:
	...
	void splice(iterator position,AdaptiveSequence<T>& x,iterator first,iterator last);
